...
- Added contributor workflows for reviewing grouped dependency updates, monitored upstream
  releases, Herdr compatibility refreshes, and temporary Ghostty Web patch removal.
...
- Added default-on Herdr prefix keybindings with a persisted toggle and one-action import from the
  selected bridge.
- Added browser mouse forwarding for terminal applications that request mouse tracking, including
...
- Added the selected bridge's Herdr theme to the web interface, including the one-dark palette.
